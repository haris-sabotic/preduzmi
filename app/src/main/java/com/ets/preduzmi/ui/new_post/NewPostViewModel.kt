...
class NewPostViewModel : ViewModel() {
    ...
    fun createBusiness(
        name: String, type: String, legalType: String, description: String, photo: String
    ) {
        val token = "Bearer ${GlobalData.getToken()}"
        Api.service.createBusiness(token, CreateBusinessRequest(
            name, type, legalType, description, photo
        )).enqueue(object : Callback<GenericResponse<BusinessModel>> {
